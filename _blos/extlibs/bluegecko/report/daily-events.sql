...
column event_severity_code Heading "Severity"
column event_duration Heading "Duration Min." format 99999999
column event_text   format a95 FOLD_BEFORE HEADING "" word_wrapped


TTITLE LEFT v_database ': ' v_title  SKIP 2

select event_id
      ,INITCAP(event_status) event_status
      ,INITCAP(event_class) event_class
      ,INITCAP(event_name) Event
      ,INITCAP(event_severity_code) event_severity_code
      ,(event_latest_occurrence - event_date_time)*24*60 event_duration
      ,event_text event_text
from bg_events 
where event_date_time > TRUNC(SYSDATE -1)
