...
SELECT 'Datafile Backup' backup_type
      , MIN(last_backup_time) recoverable_to
FROM ( SELECT f.tablespace_name, max(b.checkpoint_time) last_backup_time
...
GROUP BY 'Datafile Backup'
UNION
SELECT 'Archivelog Backup', MIN(completion_time) recoverable_to
FROM   v$archived_log
WHERE  NVL(backup_count,0) < 1
AND    status = 'A'
AND    standby_dest = 'NO'
GROUP BY 'Archivelog Backup'
UNION
SELECT 'Controlfile Backup', MAX(checkpoint_time) recoverable_to
FROM   v$backup_datafile
WHERE  file# =0
GROUP BY 'Controlfile Backup'
/
