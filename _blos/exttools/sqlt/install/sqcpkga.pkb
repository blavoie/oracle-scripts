...
/* $Header: 215187.1 sqcpkga.pkb 12.1.04 2013/11/11 carlos.sierra mauro.pagano $ */
  ...
  IS
    l_link sys.dba_db_links%ROWTYPE;
  BEGIN
    IF TRIM(p_stand_by_dblink) IS NOT NULL THEN
      s_db_link := '@'||REPLACE(UPPER(REPLACE(p_stand_by_dblink, ' ')), '@');

      BEGIN
        SELECT *
          INTO l_link
          FROM sys.dba_db_links
         WHERE db_link = REPLACE(s_db_link, '@');
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
          RAISE_APPLICATION_ERROR(-20327, 'DB Link "'||REPLACE(s_db_link, '@')||'" does not exist.');
      END;
    ELSE
      s_db_link := NULL;
    END IF;
      ...
      IF get_param('automatic_workload_repository') = 'Y' AND get_param_n('c_awr_hist_days') > 0 THEN
        IF x_sql_id IS NOT NULL THEN
          BEGIN
            ...
            'SELECT sql_text '||LF||
            '  FROM sys.dba_hist_sqltext'||s_db_link||' '||LF||
            ' WHERE dbid = :dbid '||LF||
  ...
  BEGIN
    BEGIN
      EXECUTE IMMEDIATE 'SELECT value FROM v$parameter2'||s_db_link||' WHERE LOWER(name) = LOWER(:p_name)' INTO l_value USING IN p_name;
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        l_value := NULL;
    END;
    ...
    IF l_value IS NULL THEN
      EXECUTE IMMEDIATE 'SELECT value FROM sys.sqlt$_gv$parameter_cbo_v'||s_db_link||' WHERE LOWER(name) = LOWER(:p_name)' INTO l_value USING IN p_name;
    END IF;
  ...
  BEGIN
    ...
    IF s_optimizer_mode IS NOT NULL THEN
      ...
      execute_immediate('ALTER SESSION SET optimizer_mode = '||s_optimizer_mode);
    END IF;

    IF s_optimizer_index_cost_adj IS NOT NULL THEN
      write_log('optimizer_index_cost_adj restored to "'||s_optimizer_index_cost_adj||'"');
      execute_immediate('ALTER SESSION SET optimizer_index_cost_adj = '||s_optimizer_index_cost_adj);
    END IF;
    ...
    IF s_optimizer_dynamic_sampling IS NOT NULL THEN
      ...
      execute_immediate('ALTER SESSION SET optimizer_dynamic_sampling = '||s_optimizer_dynamic_sampling);
    END IF;

    IF s_hash_join_enabled IS NOT NULL THEN
      write_log('_hash_join_enabled restored to "'||s_hash_join_enabled||'"');
      execute_immediate('ALTER SESSION SET "_hash_join_enabled" = '||s_hash_join_enabled);
    END IF;
    ...
    IF s_sortmerge_join_enabled IS NOT NULL THEN
      ...
      execute_immediate('ALTER SESSION SET "_optimizer_sortmerge_join_enabled" = '||s_sortmerge_join_enabled);
    END IF;
  ...
  BEGIN
    IF UPPER(p_directory_name) LIKE '%UDUMP%' THEN
      l_directory_path := get_v$parameter('user_dump_dest');
    ELSIF UPPER(p_directory_name) LIKE '%BDUMP%' THEN
      l_directory_path := get_v$parameter('background_dump_dest');
    ELSE
      l_directory_path := '?';
    END IF;

    IF l_directory_path LIKE '%?%' OR l_directory_path LIKE '%*%' THEN
  ...
  IS
    dir_rec sys.dba_directories%ROWTYPE;
  BEGIN
    SELECT get_v$parameter('user_dump_dest')
      INTO dir_rec.directory_path
      FROM DUAL;

    IF INSTR(dir_rec.directory_path, '\') > 0 THEN
      dir_rec.directory_path := dir_rec.directory_path||'\';
    ELSE
      dir_rec.directory_path := dir_rec.directory_path||'/';
    END IF;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      RAISE_APPLICATION_ERROR (-20320, 'directory path for USER_DUMP_DEST could not be retrieved. '||SQLERRM, TRUE);
  END get_user_dump_dest_path;
   ...
   *
   * ------------------------- */
  FUNCTION get_background_dump_dest_path
  RETURN VARCHAR2
  IS
    dir_rec sys.dba_directories%ROWTYPE;
  BEGIN
    SELECT get_v$parameter('background_dump_dest')
      INTO dir_rec.directory_path
      FROM DUAL;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      RAISE_APPLICATION_ERROR (-20320, 'directory path for BACKGROUND_DUMP_DEST could not be retrieved. '||SQLERRM, TRUE);
  END get_background_dump_dest_path;
  ...
  BEGIN
    ...
    IF s_event_10046_level IN ('12', '8', '4', '1') THEN
      ...
      execute_immediate('ALTER SESSION SET EVENTS ''10046 TRACE NAME CONTEXT FOREVER, LEVEL '||s_event_10046_level||'''', 'P');
  ...
  BEGIN
    ...
    sqlt$d.collect_gv$pq_tqstat(p_statement_id, s_statid, sqlt$d.s_insert_list_pq_tqstat, sqlt$d.s_select_list_pq_tqstat);
    ...
    IF s_event_10053_level = '1' OR s_event_10046_level IN ('12', '8', '4', '1') THEN
      IF s_statistics_level <> 'ALL' AND (get_rdbms_release >= 11 OR UPPER(get_platform) NOT LIKE '%LINUX%') THEN -- aware of 5969780
        execute_immediate('ALTER SESSION SET STATISTICS_LEVEL = '''||s_statistics_level||'''');
      END IF;
  ...
  BEGIN
    ...
    LOOP
      ...
      IF i.data_type = 'NUMBER' THEN
        EXECUTE IMMEDIATE 'SELECT COUNT(*) FROM '||i.table_name||' WHERE statement_id = :statement_id' INTO l_count USING IN p_statement_id;
        EXECUTE IMMEDIATE 'DELETE '||i.table_name||' WHERE statement_id = :statement_id' USING IN p_statement_id;
        write_log(LPAD(l_count, 8)||' rows deleted from '||i.table_name);
      ELSE
        EXECUTE IMMEDIATE 'SELECT COUNT(*) FROM '||i.table_name||' WHERE statement_id = :statement_id' INTO l_count USING IN get_statement_id_c(p_statement_id);
        EXECUTE IMMEDIATE 'DELETE '||i.table_name||' WHERE statement_id = :statement_id' USING IN get_statement_id_c(p_statement_id);
        write_log(LPAD(l_count, 8)||' rows deleted from '||i.table_name);
      END IF;
    END LOOP;
    ...
    LOOP
      l_count2 := l_count2 + 1;
      EXECUTE IMMEDIATE 'SELECT COUNT(*) FROM '||i.table_name||' WHERE statid LIKE ''%''||:statid||''%''' INTO l_count USING IN p_statement_id;
      EXECUTE IMMEDIATE 'DELETE '||i.table_name||' WHERE statid LIKE ''%''||:statid||''%''' USING IN p_statement_id;
      write_log(LPAD(l_count, 8)||' rows deleted from '||i.table_name);
    END LOOP;
  ...
  BEGIN
    ...
    LOOP
      ...
      execute_immediate('DELETE '||TOOL_REPOSITORY_SCHEMA||'.'||i.table_name);
    END LOOP;
  ...
  BEGIN
    ...
    -- possible remap
    l_schema_owner := NVL(TRIM(p_schema_owner), 'NULL'); -- NULL means no remap
    ...
    -- clean staging table since we need to delete missmatches before import
    -- be aware this table does not support concurreny
    ...
    */
    ...
    'WHERE s.statid = :statid AND s.type IN (''C'', ''I'', ''T'')';
    ...
    */
    ...
    ELSE
      ...
      IF get_rdbms_version >= '11.2.0.2' THEN -- 11.2.0.2 uses version 6
        l_version := 6;
      ...
      IF l_user_id > 0 AND l_hist_stats > 0 THEN
        ...
        IF can_access_wri THEN -- cleanup old table entries
          ...
          LOOP
            ...
            l_object_id := get_dba_object_id(l_object_type, l_schema_owner, i.table_name, i.subobject_name);
        ...
        IF can_access_wri THEN -- cleanup old index entries
          ...
          LOOP
            ...
            l_object_id := get_dba_object_id(l_object_type, l_schema_owner, i.index_name, i.subobject_name);
        ...
        IF can_access_wri THEN -- wri$_optstat_tab_history
          ...
          LOOP
            ...
            IF i.object_id <> l_prior_obj# THEN
              l_object_id := get_dba_object_id(l_object_type, l_schema_owner, i.table_name, NVL(i.subpartition_name, i.partition_name));
            END IF;
        ...
        IF can_access_wri THEN -- wri$_optstat_ind_history
          ...
          LOOP
            ...
            IF i.object_id <> l_prior_obj# THEN
              l_object_id := get_dba_object_id(l_object_type, l_schema_owner, i.index_name, NVL(i.subpartition_name, i.partition_name));
            END IF;
        ...
        IF can_access_wri THEN -- wri$_optstat_histhead_history
          ...
          LOOP
            ...
            IF i.intcol# <> l_prior_intcol# THEN
              l_column_id := get_internal_column_id(l_schema_owner, i.table_name, i.column_name, i.intcol#);
            END IF;
  ...
  FUNCTION report_sql_monitor (
    ...
    p_report_level   IN VARCHAR2 DEFAULT 'TYPICAL',
