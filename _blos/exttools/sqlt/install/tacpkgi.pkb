...
/* $Header: 224270.1 tacpkgi.pkb 11.4.5.1 2012/11/27 carlos.sierra $ */
  ...
  BEGIN /* explain_plans */
    ...
    LOOP
      BEGIN
        l_sql_text := SUBSTR(i.sql_fulltext, 1, MAX_LEN);
  ...
  BEGIN /* perform_count_star */
    ...
    LOOP
      BEGIN
        ...
        EXECUTE IMMEDIATE
        'SELECT COUNT(*) FROM "'||i.owner||'"."'||i.table_name||'" WHERE ROWNUM <= '||trca$g.g_count_star_th
        INTO l_actual_rows;
      EXCEPTION
        WHEN OTHERS THEN
          print_log(SQLERRM);
