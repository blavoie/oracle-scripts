...
/* $Header: 215187.1 sqcpkgd.pkb 12.1.03 2013/10/10 carlos.sierra mauro.pagano $ */
  ...
  s_insert_list_cellstate VARCHAR2(32767);
  s_select_list_cellstate VARCHAR2(32767);
    ...
    BEGIN
      IF sql_rec.sql_length <= 32767 THEN
        DBMS_OUTLN_EDIT.GENERATE_SIGNATURE (
           ...
           signature => sql_rec.signature_so );
      ELSE
    ...
    IF sql_rec.sql_length_unstripped - sql_rec.sql_length > 10 THEN
      ...
      BEGIN
        IF sql_rec.sql_length_unstripped <= 32767 THEN
          DBMS_OUTLN_EDIT.GENERATE_SIGNATURE (
             sqltxt    => sql_rec.sql_text_clob,
             signature => sql_rec.signature_so_unstripped );
        ELSE
  ...
  BEGIN
    ...
    LOOP
      ...
      ELSE
        x_insert_list := x_insert_list||', '||i.column_name;
        IF i.s_data_type = 'VARCHAR2' AND i.d_data_type = 'VARCHAR2' AND i.s_data_length > i.d_data_length THEN
          x_select_list := x_select_list||', SUBSTR('||l_alias||i.column_name||', 1, '||i.d_data_length||')';
        ELSIF i.s_data_type = 'LONG' AND i.d_data_type = 'CLOB' THEN
          x_select_list := x_select_list||', TO_LOB('||l_alias||i.column_name||')';
        ELSIF i.s_data_type = 'VARCHAR2' AND i.d_data_type = 'CLOB' THEN
          x_select_list := x_select_list||', TO_CLOB('||l_alias||i.column_name||')';
        ELSIF i.s_data_type = 'CLOB' AND i.d_data_type = 'VARCHAR2' THEN
          x_select_list := x_select_list||', SUBSTR(DBMS_LOB.SUBSTR('||l_alias||i.column_name||'), 1, '||i.d_data_length||')';
        ELSE
          x_select_list := x_select_list||', '||l_alias||i.column_name;
        END IF;
      END IF;
    END LOOP;
  ...
  BEGIN
    ...
    OPEN sqlarea_cv FOR
      ...
      'SELECT inst_id, hash_value, REPLACE(address||''@#'', ''@#'') address, 0 depth '||
      '  FROM gv$sqlarea'||sqlt$a.s_db_link||' '||
      ' WHERE sql_id = '''||sql_rec.xplain_sql_id||''' ';
    LOOP
  ...
  IS
    ...
    BEGIN
      ...
      LOOP
        ...
        IF p_depth < 10 THEN -- 10 levels down should be plenty
          IF objdep_rec.owner NOT IN ('CTXSYS', 'MDSYS') THEN
  ...
  BEGIN
    ...
    INSERT INTO sqlg$_pivot (object_type, object_name, object_owner, partitioned)
    SELECT 'INDEX', di.index_name object_name, di.owner object_owner, di.partitioned
      FROM sqlg$_pivot pt,
           sys.dba_indexes di
     WHERE pt.object_type = 'TABLE'
       ...
       AND pt.object_name = di.table_name
     UNION
    SELECT 'INDEX', o.object_name object_name, o.owner object_owner, di2.partitioned
      FROM sqlg$_pivot pt,
           sys.dba_indexes di,
           sys.dba_objects o,
           sys.dba_indexes di2
  ...
  BEGIN
    ...
    BEGIN
      ...
      FOR i IN (SELECT object_name, object_owner FROM sqlg$_pivot WHERE object_type = 'TABLE')
      LOOP
        BEGIN
  ...
  BEGIN
    ...
    l_insert_list||') SELECT :statement_id, :statid'||
    l_select_list||' FROM sqlt$_dba_objects x, sys.dba_policies t '||
    'WHERE x.statement_id = :statement_id AND x.object_type IN (''TABLE'', ''VIEW'', ''SYNONYM'') '||
  ...
  IS
    l_sql         VARCHAR2(32767);
  BEGIN
    IF s_rdbms_version < '10.2.0.3' THEN -- bug 5507435
      l_sql :=
  ...
  BEGIN
    IF sqlt$a.get_rdbms_release >= 11 THEN
      ...
      p_select_list||' FROM gv$cell_state'||sqlt$a.s_db_link;
      write_log(l_sql, 'S');
  ...
  BEGIN
    ...
    p_insert_list||') SELECT :statement_id, :statid'||
    p_select_list||' FROM gv$pq_tqstat'||sqlt$a.s_db_link;
    write_log(l_sql, 'S');
  ...
  BEGIN
    ...
    IF sqlt$a.s_db_link IS NULL THEN
      ...
      BEGIN
        ...
        IN 'GV$SQL_PLAN_STATISTICS_ALL', IN PLAN_FORMAT_L,
        IN p_sql_id, IN p_hash_value;
      EXCEPTION
        WHEN OTHERS THEN
          write_error(SQLERRM);
    ...
    ELSE
      ...
      BEGIN
        EXECUTE IMMEDIATE l_sql USING
        IN p_statement_id, IN p_statid, IN 'C', IN 'L',
        IN 'SQLT$_PLAN_STATISTICS_V', IN PLAN_FORMAT_L,
  ...
  BEGIN
    ...
    IF sqlt$a.s_db_link IS NULL THEN
      ...
      BEGIN
        EXECUTE IMMEDIATE l_sql USING
        IN p_statement_id, IN p_statid, IN 'C', IN 'A',
        IN 'GV$SQL_PLAN_STATISTICS_ALL', IN PLAN_FORMAT_A,
        IN p_sql_id, IN p_hash_value;
      EXCEPTION
        WHEN OTHERS THEN
          write_error(SQLERRM);
    ...
    ELSE
      ...
      BEGIN
        EXECUTE IMMEDIATE l_sql USING
        IN p_statement_id, IN p_statid, IN 'C', IN 'A',
        IN 'SQLT$_PLAN_STATISTICS_V', IN PLAN_FORMAT_A,
        IN p_statement_id, p_sql_id, IN p_hash_value;
      EXCEPTION
        WHEN OTHERS THEN
          write_error(SQLERRM);
  ...
  BEGIN
    ...
    IF sqlt$a.s_db_link IS NULL THEN
      ...
      BEGIN
        EXECUTE IMMEDIATE l_sql USING
        IN p_statement_id, IN p_statid,
        IN 'D', IN 'V', IN sql_rec.xplain_plan_hash_value,
        IN 'SQLT$_SQL_PLAN_TABLE',
        IN sqlt$a.get_statement_id_c(p_statement_id), IN PLAN_FORMAT_V;
      EXCEPTION
        WHEN OTHERS THEN
          write_error(SQLERRM);
          write_error(l_sql);
  ...
  BEGIN
    ...
    IF s_collect_perf_stats = 'Y' THEN
      s_rdbms_version:= sqlt$a.get_rdbms_version;
  ...
  BEGIN
    ...
    IF s_collect_perf_stats = 'Y' THEN
	  ...
	  -- sql performance
      collect_gv$cell_state(p_statement_id, p_statid, 'B', s_insert_list_cellstate, s_select_list_cellstate);
  ...
  BEGIN
    IF s_collect_perf_stats = 'Y' THEN
      ...
      collect_gv$cell_state(p_statement_id, p_statid, 'E', s_insert_list_cellstate, s_select_list_cellstate);
  ...
  BEGIN
    ...
    SELECT COUNT(*) INTO l_count FROM sqlt$_gv$cell_state WHERE statement_id = p_statement_id;
    write_log(l_count||' rows collected');
  ...
  IS
    ...
    l_gran_seg VARCHAR2(128);
    l_gran_tbl VARCHAR2(128);
    l_gran_idx VARCHAR2(128);
    l_gran_col VARCHAR2(128);
    l_gran_hgr VARCHAR2(128);
