...
/* $Header: 224270.1 tacpkgg.pkb 11.4.5.0 2012/11/21 carlos.sierra $ */
  ...
  BEGIN /* gather_table_stats */
    IF TO_NUMBER(g_gather_cbo_stats) > 0 AND g_tool_repository_schema LIKE 'TRCA%' THEN
      IF g_log_open THEN
        print_log('-> gather_table_stats: '||LOWER(p_table_name));
      END IF;

      SYS.DBMS_STATS.GATHER_TABLE_STATS (
        ownname          => g_tool_repository_schema,
      ...
      IF g_log_open THEN
        print_log('<- gather_table_stats: '||LOWER(g_tool_repository_schema||'.'||p_table_name));
      END IF;
    ELSE
      IF g_log_open THEN
        print_log('<- gather_table_stats: '||LOWER(g_tool_repository_schema||'.'||p_table_name||' skipped'));
      END IF;
    END IF;
  EXCEPTION
    WHEN OTHERS THEN
      IF g_log_open THEN
        print_log('gather_table_stats: '||LOWER(g_tool_repository_schema||'.'||p_table_name)||' '||SQLERRM);
      END IF;
 ...
 /*************************************************************************************/
  ...
  BEGIN /* general_initialization */
    IF g_tool_repository_schema IS NOT NULL AND NOT p_force THEN
    ...
    g_tool_repository_schema := TOOL_REPOSITORY_SCHEMA;
