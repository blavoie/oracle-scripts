...
/* $Header: 215187.1 sqcpkgt.pkb 12.1.05 2013/12/11 carlos.sierra mauro.pagano $ */
  ...
  RETURN NUMBER
  IS
    l_object_type VARCHAR2(30) := REPLACE(REPLACE(p_object_type, '_', ' '), ' SPEC');
    l_depth NUMBER;
  BEGIN
  ...
  BEGIN
    IF sqlt$a.get_param_n('colgroup_seed_secs') > 0 THEN  
      ...
      LOOP
	    ...
	    EXECUTE IMMEDIATE 'SELECT SYS.DBMS_STATS.REPORT_COL_USAGE('''||i.owner||''','''||i.table_name||''') FROM DUAL' INTO l_col_group_usage_report;
  ...
  BEGIN
    ...
    FOR i IN (SELECT 'X' row_type,
                     ...
                     END obj_type,
                     CASE
                     WHEN object_type LIKE '%TABLE%' OR object_type LIKE '%MAT%VIEW%' THEN 'tab_cols_cbo_'
                     WHEN object_type LIKE '%INDEX%' THEN 'idx_cols_cbo_'
                     END href
                FROM sqlt$_plan_extension
               WHERE statement_id = p_statement_id
               UNION ALL
