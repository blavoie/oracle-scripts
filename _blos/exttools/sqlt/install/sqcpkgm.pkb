...
/* $Header: 215187.1 sqcpkgm.pkb 12.1.05 2013/12/11 carlos.sierra mauro.pagano $ */
  ...
  TOOL_DEVELOPER         CONSTANT VARCHAR2(32)  := 'mauro.pagano';
  TOOL_DEVELOPER_EMAIL   CONSTANT VARCHAR2(32)  := 'mauro.pagano@oracle.com';
  ...
  s_gran_segm VARCHAR2(128);
  s_gran_cols VARCHAR2(128);
  s_gran_hgrm VARCHAR2(128);
...
SELECT object_owner,
       ...
       policy_type,
       long_predicate
  FROM '||LOWER(TOOL_REPOSITORY_SCHEMA)||'.sqlt$_dba_policies
 WHERE statement_id = '||s_sql_rec.statement_id||'
 ORDER BY
       object_owner,
       object_name,
...
SELECT endpoint_number,
 ...
 ORDER BY
  ...
  BEGIN
    ...
    l_row_count := 0;
    FOR i IN (SELECT *
                FROM sqlt$_dba_histgrm_stats_vers_v
               WHERE statement_id = s_sql_rec.statement_id
                 AND owner = p_owner
                 AND table_name = p_table_name
                 AND column_name = p_column_name
