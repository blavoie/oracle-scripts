...
/* $Header: 215187.1 sqcpkgi.pkb 12.1.05 2013/12/11 carlos.sierra mauro.pagano $ */
  ...
  BEGIN
    ...
    ELSE
      ...
      LOOP
        ...
        BEGIN
          EXECUTE IMMEDIATE 'BEGIN &&tool_administer_schema..sqlt$i.remote_event_10046_10053_on@'||i.db_link||'(:statement_id, :e10046); END;'
  ...
  BEGIN
    ...
    ELSE
      ...
      LOOP
        ...
        BEGIN
          EXECUTE IMMEDIATE 'BEGIN &&tool_administer_schema..sqlt$i.remote_event_10046_10053_off@'||i.db_link||'(:statement_id, :db_link, :e10046, :file_10046_10053_udump, :file_10046_10053); END;'
      ...
      LOOP
        IF i.event_10046 = 'Y' THEN
          BEGIN
            ...
            EXECUTE IMMEDIATE 'BEGIN &&tool_administer_schema..sqlt$i.remote_call_trace_analyzer@'||i.db_link||'(:statement_id, :db_link, :file_10046_10053_udump, :file_10046_10053, :out_file_identifier); END;'
            USING IN p_statement_id, IN i.db_link, IN i.file_10046_10053_udump, IN i.file_10046_10053, IN p_out_file_identifier;
          EXCEPTION
            WHEN OTHERS THEN
              write_error(SQLERRM);
              write_error('calling &&tool_administer_schema..sqlt$i.remote_call_trace_analyzer@'||i.db_link||'('||p_statement_id||', '''||i.db_link||''', '''||i.file_10046_10053_udump||''', '''||i.file_10046_10053||''', '''||p_out_file_identifier||''') failed with error above. Process continues.');
          END;
          ...
          BEGIN
           ...
           EXECUTE IMMEDIATE
           'SELECT MAX(id) FROM &&tool_repository_schema..trca$_tool_execution@'||i.db_link INTO l_tool_execution_id;
          ...
          IF l_tool_execution_id > 0 THEN
            ...
            BEGIN
              ...
              EXECUTE IMMEDIATE
              'SELECT &&tool_administer_schema..trca$i.top_sql@'||i.db_link||'(:tool_execution_id, :sys, :time, :id_type) FROM dual'
              INTO l_hash_values
        ...
        LOOP
          ...
          IF l_id IS NOT NULL THEN
            ...
            BEGIN
              ...
              EXECUTE IMMEDIATE
              'BEGIN &&tool_administer_schema..sqlt$i.remote_xtract@'||i.db_link||'(:group_id, :db_link, :file_prefix, :sql_id_or_hash_value); END;'
        ...
        BEGIN
          ...
          '  db_link, '||LF||
          '  file_text '||LF||
          'FROM &&tool_repository_schema..sqli$_file@'||i.db_link||' '||LF||
        ...
        BEGIN
          IF i.event_10046 = 'Y' THEN
            ...
            '  :db_link, '||LF||
            '  file_text '||LF||
            'FROM &&tool_repository_schema..trca$_file@'||i.db_link||' '||LF||
        ...
        BEGIN
          IF i.event_10046 = 'Y' AND l_sql_ids||l_hash_values IS NOT NULL THEN
            ...
            '  r2, '||LF||
            '  ch1 '||LF||
            'FROM &&tool_repository_schema..sqlt$_stattab@'||i.db_link||' '||LF||
        ...
        BEGIN
          ...
          '  line_id, '||LF||
          '  line_text '||LF||
          'FROM &&tool_repository_schema..sqlt$_log@'||i.db_link||' '||LF||
  ...
  BEGIN
    ...
    LOOP
      BEGIN
        l_sql := 'EXPLAIN PLAN SET statement_id = '''||l_fts||''' INTO &&tool_repository_schema..sqlt$_sql_plan_table FOR SELECT /*+ FULL(t) */ * FROM "'||i.owner||'"."'||i.table_name||'" t';
      ...
      ELSE -- it was executed by XTRACT or XECUTE, or if it was by XPLAIN and the sqltext was found in memory or AWR
        BEGIN
          ...
          IF sqlt$a.get_rdbms_version >= '11.2' THEN
            l_exportpkgbody := 'exportPkgbody => '||sqlt$a.get_param('tcb_export_pkg_body')||', ';
          ELSE
         ...
         'BEGIN SYS.DBMS_SQLDIAG.EXPORT_SQL_TESTCASE ( '||
            ...
            l_exportpkgbody||
            'samplingPercent => '||sqlt$a.get_param('tcb_sampling_percent')||', '||
            'timeLimit     => :timeLimit, '||
  ...
  BEGIN
    ...
    BEGIN
	  ...
	  SYS.DBMS_METADATA.SET_TRANSFORM_PARAM(SYS.DBMS_METADATA.SESSION_TRANSFORM, 'EXPORT', TRUE);  --12.1.05 to handle INTERVAL partitioning
    ...
    ELSE
      ...
      LOOP
        IF i.num_rows IS NULL THEN
          l_sql := 'SELECT /*+ FULL(t) PARALLEL */ COUNT(*) FROM "'||i.owner||'"."'||i.table_name||'" t WHERE ROWNUM <= :number';
        ...
        ELSIF i.num_rows < (sqlt$a.get_param_n('count_star_threshold') * 1e1) THEN
          l_sql := 'SELECT /*+ FULL(t) PARALLEL */ COUNT(*) * 1e1 FROM "'||i.owner||'"."'||i.table_name||'" SAMPLE (:number) t';
          l_number := 1e1;
        ELSIF i.num_rows < (sqlt$a.get_param_n('count_star_threshold') * 1e2) THEN
          l_sql := 'SELECT /*+ FULL(t) PARALLEL */ COUNT(*) * 1e2 FROM "'||i.owner||'"."'||i.table_name||'" SAMPLE (:number) t';
          l_number := 1e0;
        ELSIF i.num_rows < (sqlt$a.get_param_n('count_star_threshold') * 1e3) THEN
          l_sql := 'SELECT /*+ FULL(t) PARALLEL */ COUNT(*) * 1e3 FROM "'||i.owner||'"."'||i.table_name||'" SAMPLE (:number) t';
          l_number := 1/1e1;
        ELSIF i.num_rows < (sqlt$a.get_param_n('count_star_threshold') * 1e4) THEN
          l_sql := 'SELECT /*+ FULL(t) PARALLEL */ COUNT(*) * 1e4 FROM "'||i.owner||'"."'||i.table_name||'" SAMPLE (:number) t';
          l_number := 1/1e2;
        ELSIF i.num_rows < (sqlt$a.get_param_n('count_star_threshold') * 1e5) THEN
          l_sql := 'SELECT /*+ FULL(t) PARALLEL */ COUNT(*) * 1e5 FROM "'||i.owner||'"."'||i.table_name||'" SAMPLE (:number) t';
          l_number := 1/1e3;
        ELSIF i.num_rows < (sqlt$a.get_param_n('count_star_threshold') * 1e6) THEN
          l_sql := 'SELECT /*+ FULL(t) PARALLEL */ COUNT(*) * 1e6 FROM "'||i.owner||'"."'||i.table_name||'" SAMPLE (:number) t';
          l_number := 1/1e4;
        ELSIF i.num_rows < (sqlt$a.get_param_n('count_star_threshold') * 1e7) THEN
          l_sql := 'SELECT /*+ FULL(t) PARALLEL */ COUNT(*) * 1e7 FROM "'||i.owner||'"."'||i.table_name||'" SAMPLE (:number) t';
          l_number := 1/1e5;
        ELSE
          l_sql := 'SELECT /*+ FULL(t) PARALLEL */ COUNT(*) * 1e8 FROM "'||i.owner||'"."'||i.table_name||'" SAMPLE (:number) t';
          l_number := 1/1e6;
        END IF;
