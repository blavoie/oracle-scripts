...
declare
   ...
   procedure rs_report is
      ...
      l1 pls_integer := 0;       --<-- run1 latches
      l2 pls_integer := 0;       --<-- run2 latches
      n  pls_integer := 0;       --<-- report section

      -- Small function to return the difference in two timer values...
      -- --------------------------------------------------------------
      function t ( p_start in pls_integer,
                   p_end   in pls_integer ) return rt_time is
         r_time rt_time;
      begin
      ...
      procedure s ( p_stattype in st_stattype,
                    ...
                    p_r1_value in pls_integer,
                    p_r2_value in pls_integer,
                    p_diff     in pls_integer default null) is

         v_output st_output;
         v_diff   pls_integer := coalesce(p_diff, p_r2_value-p_r1_value);
