...
declare
    ...
    is
        select a.column_name,
               ...
               a.density,
               a.num_nulls,
               decode (a.histogram,  'FREQUENCY', 'Freq',  'HEIGHT BALANCED', 'HBal',  'None') as histogram,
               a.num_buckets,
               a.avg_col_len,
               a.sample_size,
               a.low_value,
...
begin
    select max (length (column_name)) + 1,
           max (length (num_distinct)) + 3,
           max (length (num_nulls)) + 1,
           max (length (num_buckets)) + 1,
    ...
    dbms_output.put_line (
           ' '
        || rpad ('Name', v_max_colname)
        || ' Analyzed             Null? '
        || rpad (' NDV', v_max_ndv)
        || '  '
        || rpad (' Density', 10)
        || rpad ('# Nulls', v_max_nulls)
        || '  '
        || rpad ('Hist.', 6)
        || '  '
        || rpad ('# Buckets', v_max_bkts)
        || '  '
        || rpad ('Sample', v_max_smpl)
        || '  AvgLen  Lo-Hi Values'
    );
    dbms_output.put_line ('=============================================================================================================================================');

    for v_rec in col_stats
    loop
        begin
            if v_rec.last_analyzed is not null
            then
                if v_rec.data_type = 'NUMBER'
                then
                    dbms_stats.convert_raw_value (v_rec.low_value, cn1);
                    dbms_stats.convert_raw_value (v_rec.high_value, cn2);
                    cv   := cn1 || ' | ' || cn2;
                elsif (v_rec.data_type = 'VARCHAR2')
                then
                ...
                elsif (v_rec.data_type = 'DATE')
                      or (substr (v_rec.data_type, 1, 9) = 'TIMESTAMP')
                then
                ...
                elsif (v_rec.data_type = 'NVARCHAR2')
                then
                ...
                dbms_output.put_line (
                       rpad (v_rec.column_name, v_max_colname)
                    || '  '
                    || v_rec.last_analyzed
                    || '  '
